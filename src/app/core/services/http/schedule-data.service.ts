...
export class ScheduleDataService extends DataService<ScheduleTransport, ScheduleListTransport> {
    ...
    addScheduleToDepartment(scheduleTransport: ScheduleTransport, departmentId: number): Observable<ScheduleTransport> {
        return this.httpClient.post<ScheduleTransport>(
            `${this.url}/departments/${departmentId}/${this.apiUrl}`,
            scheduleTransport,
        );
    }
    ...
    getScheduleForProfessor(scheduleId: number, professorId: number): Observable<ScheduleTransport> {
        return this.httpClient.get<ScheduleTransport>(`${this.url}/${this.apiUrl}/${scheduleId}/professors/${professorId}`);
    }
}
